...
4 facilities do not charge membership fee 
...
Select facid,name,member_cost,monthly_maintanence from facilities where membercost < monthly_maintanence 20% ; 

answer= badminton,table tenis,snooker,pool table. 
...
SELECT Facilities.name, Subquery.Utilization
FROM Facilities
INNER JOIN (
    SELECT facid, SUM(slots) AS Utilization
    FROM Bookings
    WHERE memid >= 1
    GROUP by facid
